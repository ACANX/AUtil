# JSONProvider 通用 REST/RPC JSON 序列化与反序列化方法提案
...
- **[M-01] 新增两个通用方法**:在 `JSONProvider` 中新增**序列化**与**反序列化**两个通用方法,作为 REST API / RPC / SDK 客户端场景下 JSON 转换的统一入口(→ §5.1)。
- **[M-02] 序列化方法默认「下划线」**:序列化输出的字段名为**下划线风格**(对应 `NamingStyle.SNAKE_CASE`),匹配 REST/RPC 传输层常见约定(→ §5.1、§5.2)。
- **[M-03] 反序列化方法默认「小驼峰」**:反序列化将下划线 JSON 字段映射回**小驼峰 Java 字段**(对应 `FieldMapping.SNAKE_TO_CAMEL`),与 Java Bean 编码风格一致(→ §5.1、§5.3)。
- **[M-04] 方法必须支持配置参数**:两个方法均需接受配置参数,调用方可按场景传入不同配置覆盖默认值(→ §5.4)。
- **[M-05] 方法需支持泛型与集合目标类型**:反序列化需支持 `Class` / `Type`(泛型)目标,满足 RPC 出参、集合响应的反序列化需求(→ §5.3)。
...
- **[E-01] 与 Feature 规范联动**:新增能力语义与 `JsonFeatureProposal.md` 的 Feature 体系完全一致,后续新增 Feature 无需改动本方法签名(→ §6)。
- **[E-02] 与 Jackson3 迁移兼容**:方法契约不绑定具体实现,`Jackson3Migration` 阶段切换默认实现时本方法签名零变化(→ §6、§8)。
- **[E-03] 向后兼容**:现有 `JSONProvider` / `JSONUtil` 方法全部保留,新方法为纯增量(→ §8.1)。
...
`JsonFeatureProposal.md` 已定义 `JsonConfig` 配置对象与 Feature 语义,本提案是其在「传输层通用方法」上的落地面。

### 2.2 目标

在 `JSONProvider` 上新增**两个通用方法**,形成面向 REST/RPC/SDK 的统一 JSON 转换入口:

- **序列化方法**:默认**下划线**输出,可按场景传入 `JsonConfig` 覆盖输出细节。
- **反序列化方法**:默认**下划线→小驼峰**映射,可按场景传入 `JsonConfig` 覆盖容错与映射细节。
...
3. **框架无关**——方法定义在 `json-core`,三个实现层各自映射到底层框架(见 `JsonFeatureProposal.md` §3)。
4. **向后兼容**——新增方法不修改、不删除、不重签名任何现有方法(见 §8.1)。
5. **与两份关联提案一致**——Feature 语义遵循 `JsonFeatureProposal.md`;实现不绑定具体框架,兼容 `Jackson3Migration` 的默认实现切换(见 §6、§8.3)。
...
在 `JSONProvider` 接口新增两个方法(对应需求 M-01~M-05):
 ...
 */
@Alpha
String toJSONStringForApi(Object object, JsonConfig config);
 ...
 */
@Alpha
<T> T parseObjectForApi(String jsonStr, Type targetType, JsonConfig config);
```

说明:

- `parseObjectForApi` 接收 `Type` 而非仅 `Class<T>`,`Class` 是 `Type` 的特例,天然覆盖 M-05 的泛型/集合需求(集合响应传 `TypeReference` / `ParameterizedType`)。
- 若未来需要"下划线数组反序列化",可在后续扩展 `parseArrayForApi`;本提案聚焦最通用的两个方法(见 §10 待办)。

`JSONUtil` 门面同步暴露静态方法,内部委托给 `PROVIDER`:

```java
public static String toJSONStringForApi(Object object) { ... }                    // 无配置,默认
public static String toJSONStringForApi(Object object, JsonConfig config) { ... }
public static <T> T parseObjectForApi(String json, Class<T> clazz) { ... }
public static <T> T parseObjectForApi(String json, Type type, JsonConfig config) { ... }
```

> **为何新增而非改造**:`toJSONStringForApi` 与 `toJSONStringSnake` 都默认下划线,但前者**接受 `JsonConfig` 且为传输层专用语义**,职责不同;改造旧方法会破坏现有调用方,违背 §3 原则 4(见 §8.1)。

### 5.2 序列化方法默认值(对应需求 M-02)
...
> **与 `parseObjectSnake` 的关系**:`parseObjectSnake(json, Class)` 等价于「`FieldMapping.SNAKE_TO_CAMEL` + 无其他配置」的简写;本方法的 `JsonConfig` 版本是其**可配置超集**,旧方法保留兼容(见 §8.1)。

### 5.4 配置参数:按场景覆盖(对应需求 M-04、C-02、C-05)
...
JsonConfig cfgC = JsonConfig.builder()
        ...
        .build();
JSONUtil.toJSONStringForApi(user, cfgC);

// 响应反序列化:默认下划线→小驼峰
User user = JSONUtil.parseObjectForApi(respJson, User.class);

// 响应反序列化:覆盖为仅同名字段严格映射
JsonConfig strict = JsonConfig.builder().fieldMapping(FieldMapping.EXACT).build();
User user2 = JSONUtil.parseObjectForApi(respJson, User.class, strict);
```

### 5.5 方法命名

- **序列化** `toJSONStringForApi`:与现有 `toJSONString*` 系列一致,`ForApi` 后缀表明"传输层通用"语义。
- **反序列化** `parseObjectForApi`:与现有 `parseObject*` 系列一致,`ForApi` 后缀同理。
- 备选:`toJSONStringForTransfer` / `parseObjectForTransfer`(更强调"传输"),`toJSONStringForRpc`(更窄)。`ForApi` 兼顾 REST + RPC + SDK,作为推荐;最终命名在评审时确认(见 §10)。
...
**依赖关系**:本提案的配置项**全部复用** Feature 提案的枚举与 `JsonConfig`,不新增平行概念;Feature 提案新增 Feature 时,本方法无需改签名(对应 E-01)。

**顺序建议**:先落地 `JsonFeatureProposal.md` 的 `JsonConfig` 与核心枚举(其阶段一),本提案即可在此之上实施。

### 6.2 与 `Jackson3Migration.md` 的关系

- `Jackson3Migration` 关注"换底层实现不破坏行为";本提案方法定义在 `json-core` 的 SPI 契约上,**不引用任何具体框架 API**。
- 本提案方法在三个实现层映射时,只依赖各自框架的**原生能力**(命名策略、include、日期适配器),与 `Jackson3Migration` 阶段一的行为快照测试**相互增强**:本方法的默认行为也应纳入快照,升级 jackson3 后同一断言必须全绿。
- `Jackson3Migration` 阶段三切换默认实现时,本提案方法签名零变化,行为由快照测试兜底(对应 E-02)。

### 6.3 依赖顺序

```
JsonFeatureProposal(JsonConfig + 枚举)  →  本提案(方法签名 + 默认语义)  →  Jackson3Migration(换实现)
```
...
以下四个场景均可由本提案两个方法覆盖;**默认配置即可满足绝大多数情况**,差异通过传入 `JsonConfig` 覆盖。

### 7.1 HTTP REST API 请求参数序列化(S-01)

```java
// 默认:下划线字段、紧凑输出、null 跳过
String body = JSONUtil.toJSONStringForApi(requestParams);
...
ApiResponse resp = JSONUtil.parseObjectForApi(respBody, ApiResponse.class);
```
...
String requestJson = JSONUtil.toJSONStringForApi(rpcRequest);
RpcResult<List<Order>> result = JSONUtil.parseObjectForApi(
...
- SDK 内部统一使用本方法作为 JSON 转换入口,默认调用即可;同时把 `JsonConfig` 作为可覆盖点暴露给使用方:

```java
public class ApiClient {
    public <T> T execute(Request req, Class<T> respClass) {
        String body = JSONUtil.toJSONStringForApi(req, this.globalConfig);
        String respBody = http.post(uri, body);
        return JSONUtil.parseObjectForApi(respBody, respClass, this.globalConfig);
    }
...
- **零破坏**:新增两个接口方法与对应 `JSONUtil` 静态方法,不修改、不删除、不重签名现有任何方法。
- **旧方法保留**:`toJSONStringSnake` / `parseObjectSnake` 等继续可用,与新方法并存;新方法是其"可配置超集",旧方法内部可委托到等价 `JsonConfig` 组合(与 `JsonFeatureProposal.md` §8.3 一致)。
- **实现层**:三个 Provider 若未立即实现新方法,可在接口上提供 `default` 实现(委托到现有方法 + 忽略 `JsonConfig`),保证编译不破;完整支持由后续 Feature 阶段补齐。
...
| 新方法与 `*Snake` 系列职责重叠引发困惑 | 低 | 低 | §5.1 命名与文档说明职责边界 |

### 8.3 与 Jackson3 迁移叠加

- 本方法在 `json-jackson`(Jackson 2)实现落地后,`json-jackson3` 需通过同一套快照断言(见 `Jackson3Migration.md` 阶段一/阶段二)。
- `Jackson3Migration` 阶段三的显式优先级表已保证 `jackson3 > jackson2 > gson > fastjson2`,本方法的行为一致性由快照兜底。

---

## 9. 验收标准

- [ ] `JSONProvider` 新增两个方法签名,`JSONUtil` 暴露对应静态方法
- [ ] 不传配置时:序列化输出下划线、紧凑、null 跳过;反序列化下划线→小驼峰、忽略未知字段(行为快照测试固化)
...
1. 评审本提案,确认方法命名(`toJSONStringForApi` / `parseObjectForApi`)与配置语义。
